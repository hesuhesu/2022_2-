...
public class SuController2 {
	...
	public String depositPost(HttpSession session, int choice, int money, Model mo) {
		...
		mo.addAttribute("msg", money + "원이 " + (choice==1?"입급":"출금")+"완료 되었습니다. (거래 내역 화면으로 이동)");
	...
	public String transferNew(HttpSession session, int money, String tid, Model mo) {
		...
		if(id.equals(tid)) {
			...
			mo.addAttribute("msg", tid+"님께 "+money + "원이 이체 완료되었습니다. (거래 내역 화면으로 이동)");
			mo.addAttribute("url", "/myhistory");
		}
		else {
			mo.addAttribute("msg", "미동록 수신인 아이디입니다. 수신인 아이디를 확인해 주세요.");
			mo.addAttribute("url", "back");
		}
		return "popups";
	}
