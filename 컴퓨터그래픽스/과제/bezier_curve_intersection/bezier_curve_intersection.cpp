...
void DrawIntersectionPoints(BVH* bvh0, BVH* bvh1)
{
    glColor3f(0, 1, 0);
    glPointSize(5.0);
    for (auto& bv0 : bvh0->roots) {
        for (auto& bv1 : bvh1->roots) {
            // BV의 AABB가 겹치는지 확인
            if (bv0.min.x <= bv1.max.x && bv0.max.x >= bv1.min.x &&
                bv0.min.y <= bv1.max.y && bv0.max.y >= bv1.min.y) {
                // AABB가 겹치면 BV의 교점을 찾음
                std::vector<Vec2> bv0_pts = bv0.pts;
                std::vector<Vec2> bv1_pts = bv1.pts;
                for (size_t i = 0; i < bv0_pts.size(); ++i) {
                    for (size_t j = 0; j < bv1_pts.size(); ++j) {
                        if (std::abs(bv0_pts[i].x - bv1_pts[j].x) < 0.1 && std::abs(bv0_pts[i].y - bv1_pts[j].y) < 0.1) {
                            // 벡터의 차가 threshold(여기서는 0.1) 이하인 경우 교점으로 간주
                            glBegin(GL_POINTS);
                            glVertex2f(bv0_pts[i].x, bv0_pts[i].y);
                            glEnd();
                        }
                    }
                }
...
void display() {
    ...
    //Draw intersection points between two curves using the BVH
    DrawIntersectionPoints(bvh0, bvh1);
