...
void DrawComponent::Draw()
{
    ...
    // auto vc = mesh.get_vertex_property<pmp::Normal>("v:color");
    ...
    if(true){
        ...
        std::set<pmp::Vertex> visitedVextexes; // startV 의 집합. 점의 집합 - 1
        for (auto f : mesh.vertices(startV)) {
            visitedVextexes.insert(f);
        }

        std::set<pmp::Vertex> visitedVextexes2; // 점의 집합 - 2
        glColor3f(1, 1, 1);
        glBegin(GL_POINTS);
        for (auto f : visitedVextexes) {
            for (auto k : mesh.vertices(f)) {
                for (auto t : mesh.vertices(startV)) {
                    if (t == k) continue;
                    auto p = mesh.position(k);
                    // glVertex3dv(p.data()); // -> 해당 부분은 vertex 입니다. 점을 볼려면 주석 해제
                    visitedVextexes2.insert(k); // 하는 김에 두 번째 집합에 추가
                }
            }
        }
        glEnd();

        if (true) { // 2-ring face. 초록 면
            glColor3f(0, 1, 0);
            for (auto f : visitedVextexes2) {
                for (auto k : mesh.faces(f)) {
                    DrawFace(k);
                }
            }
        }

        std::set<pmp::Vertex> visitedVextexes3; // 점의 집합 - 3
        glColor3f(0, 0, 0);
        glBegin(GL_POINTS);
        for (auto f : visitedVextexes2) {
            for (auto k : visitedVextexes) {
                for (auto f2 : mesh.vertices(f)) {
                    for (auto k2 : mesh.vertices(k)) {
                        if (f2 == k2) continue;
                        auto p = mesh.position(f2);
                        // glVertex3dv(p.data()); // -> 해당 부분은 vertex 입니다. 점을 볼려면 주석 해제
                        visitedVextexes3.insert(f2);
                    }
                }
            }
        }
        ...
        if (true) { // 3-ring face. 보라색 면.. 마지막
            glColor3f(1, 0, 1);
            for (auto f : visitedVextexes3) {
                for (auto k : mesh.faces(f)) {
                    DrawFace(k);
                }
            }
